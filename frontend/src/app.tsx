...
import { AuthForm } from './components/AuthForm'; // Importação do formulário de autenticação
...
export default function App() {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem('@UPStudent:token'));
  const [editando, setEditando] = useState<{diaId: string, aula: Aula} | null>(null);

  // Recarrega os dados da grade assim que um token válido for detectado (login feito)
  useEffect(() => {
    if (token) {
      carregarDados();
    }
  ...
  const handleLogout = () => {
    localStorage.removeItem('@UPStudent:token');
    setToken(null);
  };

  // 1. Bloqueio de segurança: Se não houver token, exibe a tela de login/cadastro
  if (!token) {
    return <AuthForm onAuthSuccess={(newToken) => setToken(newToken)} />;
  }

  // 2. Tela de erro de conexão caso a API falhe
      ...
      <header className="bg-stone-200 border-b-2 border-stone-300 px-6 py-8 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Botão Sair integrado de forma harmoniosa com o estilo dos outros botões */}
            <button 
              onClick={handleLogout}
              className="px-3 py-1.5 border-2 border-red-700 text-red-700 font-bold rounded-md hover:bg-red-700 hover:text-white transition-all text-xs active:scale-90"
